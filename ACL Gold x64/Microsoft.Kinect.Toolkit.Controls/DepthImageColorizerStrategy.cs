...
namespace Microsoft.Kinect.Toolkit.Controls
{
    ...
    internal class DepthImageColorizerStrategy
    {
        ...
        /// </param>
        public virtual void ColorizeDepthPixels(IDepthImageColorizerParameters parameters, Object[] depthImagePixels, byte[] colorBuffer, int depthWidth, int depthHeight, int downscaleFactor)
        {
                ...
                fixed (byte* colorBufferPtr = colorBuffer)
                {
                    fixed (DepthImagePixel* depthImagePixelPtr = depthImagePixels)
                    {
                        fixed (int* playerColorLookupPtr = this.playerColorLookupTable)
                        {
                            ...
                            Object* currentPixelRowPtr = depthImagePixelPtr;

                            for (int row = 0; row < depthHeight; row += downscaleFactor)
                            {
                                Object* currentPixelPtr = currentPixelRowPtr;
